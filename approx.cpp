...
namespace APPROX{
                   ...
                   bool is_closed0, double eps, AutoBuffer<Range>& _stack )
    {
        ...
        /*int k{0};
        Point prev, next, cur;
        double successive_inner_product;
        for (i = 0; i < count; ++i) {
            next = dst_contour[(i + 1) % count];
            prev = k == 0 ? dst_contour[count - 1] : dst_contour[k - 1];
            cur = dst_contour[i];

            successive_inner_product = (cur - prev).dot(next - cur);
            auto hSqr = (next - prev).dot(next - prev);
            auto area = (cur - prev).cross(next - cur);

            if (area * area < hSqr * 0.5 * eps && successive_inner_product >= 0) continue;
            dst_contour[k++] = cur;
        }
